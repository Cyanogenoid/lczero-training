...
class TFProcess:
    ...
    def init_net(self, next_batch):
        ...
        self.lr_at_step, scheduled_steps = self.create_lr_scheduler(self.cfg['training']['lr_schedule'])
        assert scheduled_steps >= self.cfg['training']['total_steps']
        self.lr = self.lr_at_step(0)
    ...
    def process(self, batch_size, test_batches, batch_splits=1):
        ...
        self.lr = self.lr_at_step(steps_total)
    ...
    def create_lr_scheduler(self, schedule, starting_lr=0):
        """Construct a piecewise function that given a step returns the corresponding learning rate
        """

        # each of these returns a function that, given the ratio, returns a learning rate
        ...
        def repeat(entry, previous_lr):
            scheduler, steps = self.create_lr_scheduler(entry['schedule'], previous_lr)
            times = entry['times']
            entry['steps'] = steps * times
            return lambda ratio: scheduler(int(ratio * steps * times) % steps) if ratio < 1 else scheduler(steps)
