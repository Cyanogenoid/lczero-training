...
def log_session(session, writer):
    ...
    writer.add_scalar('loss/value_z', session.metrics['value_z_loss'], global_step=session.step)
    writer.add_scalar('loss/value_q', session.metrics['value_q_loss'], global_step=session.step)
    ...
    writer.add_scalar('metrics/value_accuracy', session.metrics['value_z_accuracy'], global_step=session.step)
    writer.add_scalar('metrics/value_q_accuracy', session.metrics['value_q_accuracy'], global_step=session.step)
